    ...
    GetDrawIcon: function (id) {
        ...
        if (g_JSComplierResource.DrawIcon.Data.has(id)) {
            ...
            icon = {
                Symbol: iconfont.Text,
                Color: iconfont.Color,
                Family: g_JSComplierResource.DrawIcon.Family,
                IconFont: true,
                ID: id
            };
            return icon;
...
function Node() {
    ...
    {
        let jobs = [];
        if (this.IsNeedSymbolData) jobs.push({
            ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SYMBOL_DATA
        });
        if (this.IsNeedIndexData) jobs.push({
            ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_INDEX_DATA
        });
        if (this.IsNeedLatestData) jobs.push({
            ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SYMBOL_LATEST_DATA
        });
        if (this.IsNeedSymbolDayData) jobs.push({
            ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SYMBOL_DAY_DATA
        });
        if (this.IsNeedLatestIndexData) jobs.push({
            ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_LATEST_INDEX_DATA
        });
        ...
        for (var jobID of this.IsNeedFinanceData) {
            if (jobID == JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_DIVIDEND_YIELD_DATA) //股息率 需要总市值
                jobs.push({
                    ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARKETVALUE_DATA
                });
        ...
        for (var item of this.IsNeedSectionFinance) {
            jobs.push({
                ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SECTION_SF,
                SF: item
            });
        }
    ...
    this.VerifySymbolVariable = function (varName, token) {
        ...
        if (g_JSComplierResource.IsCustomVariant(varName)) //自定义函数( 不过滤了, 调一次就写一次)
        {
            var item = {
                Name: varName,
                ID: JS_EXECUTE_JOB_ID.JOB_CUSTOM_VARIANT_DATA
            }
    ...
    this.VerifySymbolFunction = function (callee, args, token) {
        ...
        if (g_JSComplierResource.IsCustomFunction(callee.Name)) {
            var item = {
                Name: callee.Name,
                ID: JS_EXECUTE_JOB_ID.JOB_CUSTOM_FUNCTION_DATA,
                Args: args
            }
            if (token) item.Token = {
                Index: token.Start,
        ...
        {
            var item = {
                ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_HK_TO_SHSZ,
                Args: args,
        ...
        {
            var item = {
                ID: JS_EXECUTE_JOB_ID.JOB_EXECUTE_INDEX,
                Args: args,
                FunctionName: callee.Name
            };
            if (token) item.Token = {
        ...
        if (callee.Name == "LOADAPIDATA") //加载API数据
        {
            var item = {
                Name: callee.Name,
                ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_CUSTOM_API_DATA,
                Args: args
            };
            if (token) item.Token = {
                Index: token.Start,
        ...
        if (setStockDataName.has(callee.Name)) {
            var item = {
                Name: callee.Name,
                ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_OTHER_SYMBOL_DATA,
                Args: args
            };
            if (token) item.Token = {
                Index: token.Start,
...
function JSAlgorithm(errorHandler, symbolData) {
    ...
    this.CalculateZIGLine = function (firstData, secondData, thridData, data, result) {
        ...
        if (isUp) {
            for (var i = firstData.ID + 1; i < thridData.ID; ++i) //查找最高点
            {
                ...
                if (findData.Value < subItem) findData = {
                    ID: i,
                    Value: subItem
                };
            }
    ...
    */
    ...
    this.CoverPeriod = function (name, periodName, n) {
        ...
        var curPeriodInfo = this.GetPeriodInfo({
            PeriodID: this.SymbolData.Data.Period
        });
    ...
    */
    ...
    this.CallCustomFunction = function (name, args, symbolData, node) {
        ...
        var obj = {
            ...
            KData: symbolData.Data,
            ThrowError: function (message) {
                self.ThrowUnexpectedNode(node, message);
            }
        };
    ...
    */
    this.DRAWBAND = function (data, color, data2, color2) {
        ...
        let result = {
            DrawData: drawData,
            DrawType: 'DRAWBAND',
            Color: [color.toLowerCase(), color2.toLowerCase()]
        }; //颜色使用小写字符串
        let count = Math.max(data.length, data2.length);
...
JSDraw.prototype.CalculateDrawLine = function (lineCache) {
    ...
    var result = [];
    result.push({
        ID: lineCache.Start.ID,
        Value: lineCache.Start.Value
    }); //第1个点
                ...
                var obj = {
                    ...
                    Request: {
                        ...
                        Data: {
                            ...
                            symbol: '000001.sh',
                            start: -1,
                            count: self.MaxRequestDataCount + 500
                        }
                    },
                    Self: this,
                    PreventDefault: false
                ...
                var obj = {
                    ...
                    Request: {
                        ...
                        Data: {
                            ...
                            symbol: '000001.sh',
                            start: -1,
                            count: self.MaxRequestDataCount + 5
                        }
                    },
                    Self: this,
                    PreventDefault: false
    ...
    this.GetIndexIncreaseData = function (job) {
        ...
        if (this.DataType === HQ_DATA_TYPE.MINUTE_ID || this.DataType === HQ_DATA_TYPE.MULTIDAY_MINUTE_ID) //走势图数据
        {
            ...
            JSNetwork.HttpRequest({
                ...
                async: true,
                success: function (data) {
                    self.RecvMinuteIncreaseData(data, {
                        UpKey: upKey,
                        DownKey: downKey
                    });
                    self.Execute.RunNextJob();
        ...
        } else if (this.DataType === HQ_DATA_TYPE.KLINE_ID && ChartData.IsDayPeriod(this.Period, true)) //K线图 日线
        {
            ...
            JSNetwork.HttpRequest({
                ...
                async: true,
                success: function (data) {
                    self.RecvHistoryIncreaseData(data, {
                        UpKey: upKey,
                        DownKey: downKey
                    });
                    self.Execute.RunNextJob();
            ...
            if (this.NetworkFilter) {
                ...
                if (this.KLineDateTimeRange) {
                    obj.Request.KLineDataTimeRange = {
                        Start: {
                            Date: this.KLineDateTimeRange.Start.Date
                        },
                        End: {
                            Date: this.KLineDateTimeRange.End.Date
                        }
                    };
...
function JSExecute(ast, option) {
    ...
    this.ReadCustomVariant = function (name, node) {
        ...
        var obj = {
            ...
            KData: this.SymbolData.Data,
            Period: this.SymbolData.Period,
            Right: this.SymbolData.Right
        };
    ...
    this.Run = function () {
        try {
            ...
            if (error.Job) {
                ...
                this.JobList.push({
                    ID: JS_EXECUTE_JOB_ID.JOB_RUN_SCRIPT
                });
                this.Execute();
...
JSComplier.Execute = function (code, option, errorCallback) {
    ...
    var asyncExecute = function () {
        try {
            ...
            if (option.ClassName == 'ScriptIndexConsole') execute.JobList.unshift({
                ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SYMBOL_DATA
            });
            execute.JobList.push({
                ID: JS_EXECUTE_JOB_ID.JOB_RUN_SCRIPT
            });
...
function ScriptIndex(name, script, args, option) {
    ...
    this.BindInstructionData = function (hqChart, windowIndex, hisData) //绑定指示指标
    {
        ...
        if (this.InstructionType == 2) //五彩K线
        {
            ...
            hqChart.SetInstructionData(this.InstructionType, {
                Data: varItem.Data,
                Name: this.Name,
                Param: indexParam,
                ID: this.ID
        ...
        for (var i in outVar) {
            ...
            if (item.data) {
                ...
                } else if (draw.DrawType == 'STICKLINE') //柱子
                {
                    ...
                    drawItem.DrawType = draw.DrawType;
                    drawItem.DrawData = {
                        Icon: this.FittingMultiText(draw.DrawData.Icon, date, time, hqChart),
                        Family: draw.DrawData.Family
                    };
                    outVarItem.Draw = drawItem;
...
function ScriptIndexConsole(obj) {
    ...
    this.RecvResultData = function (outVar, param) {
        ...
        var result = {
            ...
            Stock: {
                Name: jsExec.SymbolData.Name,
                Symbol: jsExec.SymbolData.Symbol
            }, //股票信息
            Index: {
                Name: self.Name,
                ID: self.ID
